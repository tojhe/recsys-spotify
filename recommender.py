...
class recommender():
    ...
    def user_get_playlist_track_audio_feat(self):
        ...
        for i,song in enumerate(self.track_list):
            ...
            song_audio_features[song]['artist_popularity'] = self.spotify.artist(track_info['artists'][0]['uri'])['popularity']
    ...
    def user_playlist_recommend_tracks(self):
        ...
        numeric_feature = pickle.load(open('./pickles/new_feature_to_scale.sav','r'))
        for col in df.columns:
            if col in numeric_feature:
                df[col] = pd.to_numeric(df[col],errors ='coerce')
        
        ## Standard Scaler
        ss = pickle.load(open('./pickles/new_scaler.sav','r'))
        ...
        ## Songs database
        database_df = pickle.load(open('./pickles/new_audio_features.sav','r'))
        ...
        for i, track in df.iterrows():   ## df is the current playlist to recommend
            ...
            if len(track['artist_genres']) >=5:
                tracks_three = database_df['artist_genres'].apply(set).apply(lambda x: x.intersection(set(track['artist_genres']))).apply(lambda y: len(y) >= 3)
                genre_tracks.extend(database_df[tracks_three].index.tolist())


            else:
                ## For niche artist
                tracks_one = database_df['artist_genres'].apply(set).apply(lambda x: x.intersection(set(track['artist_genres']))).apply(lambda y: len(y) >= 1)
                genre_tracks.extend(database_df[tracks_one].index.tolist())



            ## Computing cosine similarity for each track within the same genre
            cos_dist = cosine_similarity(pd.DataFrame(track[:-3]).T, database_df[database_df.index.isin(genre_tracks)].iloc[:,:-3])
            cos_dist = pd.Series(cos_dist.reshape(-1), index = database_df[database_df.index.isin(genre_tracks)].iloc[:,:-3].index)
    ...
    def user_playlist_add_tracks(self,playlist_id=None,tracks=None,num_tracks_add='max'):
        ...
        self.user.user_playlist_add_tracks(self.username,playlist_id=playlist_id,tracks=tracks)
        print ('tracks added')